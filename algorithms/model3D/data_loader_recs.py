...
class Data_Loader:
    def __init__(self, options, train, test, session_key, item_key, time_key, pad_test=True):
        ...
        # Calculate the maximum number of items in a training session...
        max_session_length_train = train.groupby(session_key).size().max()
        
        # Calculate the numbers of items in a testing session
        ...
        # train_padded is dataframe... where data looks like this... session_id [item_id1, item_id2, item_id3, item_id1].. 34 [3425, 4365, 32456, 7574]
        train_padded = (train.groupby(session_key)[item_key]
                        .apply(lambda x: list(x))
                        .reset_index())

        # train_padded.to_csv("abc.csv", index = False)
        # print("train_padded::::::", train_padded.columns)
        # print("train_padded::::::", type(train_padded))
        ...
        #train_padded.to_csv("abcc.csv", index = False)
        '''
        ...
        #train_padded.to_csv("abccd.csv", index = False)
    ...
    def transform_test(self, test, session_key, item_key,time_key):
        ...
        ab = np.array(list(self.vocab_processor.fit_transform(positive_examples)))
        print("ab::::::::::     ",   ab)
