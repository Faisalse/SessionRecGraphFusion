...
import algorithms.model3D.data_loader_cosmec as data_loader_cosmec
import time
import pandas as pd
import numpy as np
...
class Model3D:
    ...
    def fit(self, train, test):
        ...
        self.dataKoaderObject = data_loader_cosmec.Data_Loader(train, test)
        ...
        self.All_items =  pd.unique(self.targetItems)
    ...
    def predict_next(self, session_id, prev_iid, prev_cat,  prev_brand):
        if(session_id != self.session):
            ...
            self.session_sequence_cat = list()
            self.session_sequence_brand = list()
            self.session = session_id
            
        self.session_sequence_id.append(prev_iid)
        self.session_sequence_cat.append(prev_cat)
        self.session_sequence_brand.append(prev_brand)
        
        idd = self.session_sequence_id.copy()
        cat = self.session_sequence_cat.copy()
        brand = self.session_sequence_brand.copy()
        ...
        if(len(idd) < length):
            ...
            cat[:0] = [0] * (length - len(cat))
            brand[:0] = [0] * (length - len(brand))
        elif(len(idd) > 6): 
            idd = idd[:7]
            cat = cat[:7]
            brand = brand[:7]
        ...
        prod_cat_slicing  = self.dataKoaderObject.Sessionsequence(pd.DataFrame([cat]))
        brand_slicing     = self.dataKoaderObject.Sessionsequence(pd.DataFrame([brand])) 
