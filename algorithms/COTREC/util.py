...
class Data():
    def __init__(self, data, all_train, shuffle=False, n_node=None, seed = 2000):
        self.seed = seed
        init_seed(self.seed)
        
        self.raw = np.asarray(data[0])
        adj = self.data_masks(all_train, n_node)
    ...
    def get_slice(self, index):
        ...
        diff_mask = np.ones(shape=[len(session_len), self.n_node]) * (1/(self.n_node - 1))
    ...
    def data_masks(self, all_sessions, n_node):
        init_seed(self.seed)
