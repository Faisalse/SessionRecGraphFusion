...
class AttributeAggregator(nn.Module):
    def __init__(self, dim, alpha, dropout):
        ...
        for k in range(self.attribute_kinds):
            ...
            self.Ws.append(w)
        self.combine_NN = nn.Linear(self.dim * self.attribute_kinds, self.dim, bias=False)
    ...
    def generate_mirror(self, alpha, as_items, attribute_mask):
        as_representations = []
        for i in range(self.attribute_kinds):
            # preprocess
