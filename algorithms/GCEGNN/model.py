...
from algorithms.GCEGNN.aggregator import LocalAggregator, GlobalAggregator
from torch.nn import Module, Parameter
import torch.nn.functional as F


class CombineGraph(Module):
    def __init__(self, lr, batch_size, hidden_size, session_length, number_hop, dropout_local, dropout_global, activate, num_node, adj_all, num, l2):
        ...
        # Aggregator
        self.local_agg = LocalAggregator(self.dim, self.alpha, dropout=0.0)
        self.global_agg = []
        for i in range(self.hop):
            if self.activate == 'relu':
                agg = GlobalAggregator(self.dim, self.dropout_gcn, act=torch.relu)
            else:
                agg = GlobalAggregator(self.dim, self.dropout_gcn, act=torch.tanh)
