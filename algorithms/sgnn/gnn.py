...
class Model(object):
    def __init__(self, embedding_size = 100, out_size = 100, batch_size=100, nonhybrid=True, seed = 2000):
        self.seed = seed
        init_seed(self.seed)
...
class GGNN(Model):
    def __init__(self, embedding_size=100, out_size=100, batch_size=100, max_sess_length=100, 
                 lr=0.001, l2=0.00001, step=1, lr_dc=0.1, lr_dc_step=3, nonhybrid=True, epoch=10, batch_predict=False, seed = 2000):
        
        super(GGNN,self).__init__(embedding_size, out_size, batch_size, nonhybrid, seed)
    ...
    def fit(self, train, test=None, sample_store=10000000):
        ...
        self.train_data = Data(train_data, sub_graph=True, method=method, shuffle=True, seed = self.seed)
        self.test_data = Data(test_data, sub_graph=True, method=method, shuffle=False, seed = self.seed)
        ...
        with tf.Graph().as_default():
            ...
            for epoch in tqdm(range(self.epoch_n)):
            # print('epoch: ', epoch, '===========================================')
