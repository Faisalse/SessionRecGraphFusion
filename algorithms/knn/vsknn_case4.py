...
class VMContextKNN:
    ...
    def __init__( self, k, sample_size=1000, sampling='recent', similarity='vec', weighting='div', dwelling_time=False, last_n_days=None, last_n_clicks=None, remind=True, push_reminders=False, add_reminders=False, extend=False, weighting_score='div', weighting_time=False, normalize=True, idf_weighting=False, idf_weighting_session=False, session_key = 'SessionId', item_key= 'ItemId', time_key= 'Time', cat_key='CatId', add_cate_info = True, picked_items = 500, give_importance=10):
        self.k = k
...
#         print( 'cknn.predict_next: ', process.memory_info().rss, ' memory used')
        ...
        if self.add_cate_info:
            ...
            series = self.shuffle_items_order_based_category(self.session_items, series, self.picked_items)
        ...
        return series 
    def shuffle_items_order_based_category(self, current_session_items, sorted_ranking_list, picked_items):
        sorted_ranking_list = sorted_ranking_list[:picked_items]
        if len(current_session_items) > 0:
            num_items_in_session = len(current_session_items)
            for item in reversed(current_session_items):
                ...
                import_factor = num_items_in_session/len(current_session_items)
                ...
                # decrease the value of import_facotr in reverse order.
                num_items_in_session = num_items_in_session - 1
