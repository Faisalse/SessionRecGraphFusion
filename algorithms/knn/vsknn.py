...
class VMContextKNN:
    ...
    def __init__( self, k, sample_size=1000, sampling='recent', similarity='vec', weighting='div', dwelling_time=False, last_n_days=None, last_n_clicks=None, remind=True, push_reminders=False, add_reminders=False, extend=False, weighting_score='div', weighting_time=False, normalize=True, idf_weighting=False, idf_weighting_session=False, session_key = 'SessionId', item_key= 'ItemId', time_key= 'Time', cat_key='CatId', price_key = "Price", add_cate_info = True, picked_items = 200):
        self.k = k
    ...
    def fit(self, data, items=None):
        ...
        index_price = train.columns.get_loc( self.price_key )
        session = -1
        session_items = set()
        time = -1
        #cnt = 0
        for row in train.itertuples(index=False):
            ...
            if row[index_item] not in self.item_price_map:
                self.item_price_map[row[index_item]] = row[index_price]
    ...
    def shuffle_items_order_based_category(self, current_session_items, sorted_ranking_list, picked_items):
        ...
        if len(current_session_items) > 0:
            ...
            for item in current_session_items:
                ...
                if category in ranking_category:
                    ranking_category[category] +=1
                else:
                    ranking_category[category] = 1
                    
            for cate in ranking_category:
                ranking_category[cate]/= len(current_session_items)
                
            for item in sorted_ranking_list.index:
                 category = self.check_category(item)
                 if category in ranking_category:
                     score = ranking_category[category]
                     sorted_ranking_list[item] = sorted_ranking_list[item] + score   
