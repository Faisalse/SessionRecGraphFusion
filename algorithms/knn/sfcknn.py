...
class SeqFilterContextKNN:
    ...
    def __init__( self, k = 200, sample_size=300, sampling='recent',  similarity = 'cosine', weighting_score = "linear", remind=False, pop_boost=0, extend=False, 
                 normalize=True, session_key = 'SessionId', item_key= 'ItemId', time_key= 'Time' ):
            ...
            if row[index_session] != session:
                ...
                if last_item != -1: #fill followed by map for filtering of candidate items
                    if not last_item in self.followed_by: 
                        self.followed_by[ last_item ] = list()
                    self.followed_by[ last_item ].append( row[ index_item ] )
        ...
        for session in neighbors:
            ...
            for item in items:
                ...
                if input_item_id in self.followed_by and item in self.followed_by[input_item_id]: #hard filter the candidates
                    ...
                    # after_items = self.followed_by[input_item_id]
                    
                    # getIndex = after_items.index(item) + 1
                    
                    # decay = getattr(self, self.weighting_score+'_score')( getIndex )
                    # print("Decay   ", decay)
