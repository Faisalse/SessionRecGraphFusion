...
class VMContextKNN:
    ...
    def __init__( self, k, sample_size=1000, sampling='recent', similarity='vec', weighting='div', dwelling_time=False, last_n_days=None, last_n_clicks=None, remind=True, push_reminders=False, add_reminders=False, extend=False, weighting_score='div', weighting_time=False, normalize=True, idf_weighting=False, idf_weighting_session=False, session_key = 'SessionId', item_key= 'ItemId', time_key= 'Time', cat_key='CatId', add_cate_info = True, picked_items = 0, give_importance=0, rec_popularity=True, lookback = [3,4,4]):
        self.k = k
        ...
        self.rec_popularity = rec_popularity
    ...
    def fit(self, data, items=None):
        ...
        if self.last_n_days != None:
            ...
            self.train_f = data[ data[self.time_key] >= stamp ]
...
#         print( 'cknn.predict_next: ', process.memory_info().rss, ' memory used')
        ...
        if self.add_cate_info:
            ...
            series = self.shuffle_items_order_based_category(self.session_items, series, self.picked_items, self.give_importance)
        
        if self.rec_popularity:
            
            series = self.calculate_pop(series, self.train_f, timestamp)
        
        if self.normalize:
            series = series / series.max()
        return series 
    def shuffle_items_order_based_category(self, current_session_items, sorted_ranking_list, picked_items, give_importance):
        self.sessionAnalysis(current_session_items)
        self.rankingListAnay(sorted_ranking_list)
        ...
        for item in reversed(current_session_items):
            # Calculate the importance factor and give more importance to last item of curren session.
            import_factor = 1/2**counter
            
            # find the category of selected item
            category = self.check_category(item)
            
            # find the items with this category
            ...
            for items_same in items_with_same_category:
                if items_same in sorted_ranking_list.index:
                    sorted_ranking_list[items_same] = sorted_ranking_list[items_same] + import_factor
                
        return sorted_ranking_list 

    
        # calculate item importance categories wise, for example, in current sessions, 4 items belong
        # category A, 3 belongs category B and 1 belongs to category C, so, we give importance to itms
        # of category A, then B and then A, score can be calculated as 4/8, 3/8, 1/8, 8 is current 
        # session length, however, in this case, we did not see any imporvement. 
# =============================================================================
#         if len(current_session_items) > 0:
#             current_session_last_item = list(current_session_items)[-1]
#             category = self.check_category(current_session_last_item)
#             items_with_same_category = list(set(self.find_item_of_category(category)))
#             for items_same in items_with_same_category:
#                 if items_same in sorted_ranking_list.index:
...
# =============================================================================
    ...
    def calculate_pop(self, ranking_list, train, timestamp):
        #ranking_list = ranking_list[:50]
        session_occuring_time = timestamp
        look_back_days = [1, 3,7,14]
        for days in look_back_days:
            for item in ranking_list.index:
                dataset = self.train_f.iloc[:2, :]
                for i in range(days):
                    frame = [dataset, self.train_f[train[self.time_key] == (session_occuring_time - ((i+7) *86400 ))]]
                    dataset = pd.concat(frame)
                total_view = dataset.shape[0]
                itemviews = dataset[dataset[self.item_key] == item]
                itemviews = itemviews.shape[0]
                ranking_list[item] += (itemviews / total_view)
        print("Faisal")
        return ranking_list
...
#                 pos_map[item] = pos_map[item] / max
    ...
    def sessionAnalysis(self, currentSession):
        ...
        for i in currentSession:
            items_list.append(i)
            category_list.append(self.item_cat_map[i])
            
        dataframe = pd.DataFrame()
        dataframe["item"] = items_list
        dataframe["category"] = category_list
        dataframe.to_csv(str(self.count)+"currentsession.csv")
    
     
    def rankingListAnay(self, rankinglist):
        counter = 1
        ...
        dataframe["item"] = items_list
        dataframe["category"] = category_list
        dataframe.to_csv(str(self.count)+"ranking.csv")
        self.count +=1 
