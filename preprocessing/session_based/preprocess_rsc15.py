...
DATA_PATH = r'./data/rec15/raw/'
DATA_PATH_PROCESSED = r'./data/rec15/prepare/'
#DATA_FILE = 'yoochoose-clicks-10M'
DATA_FILE = 'rsc15_clicks'
...
def preprocess_only(path=DATA_PATH, file=DATA_FILE, path_proc=DATA_PATH_PROCESSED, test_days = DAYS_TEST, min_item_support=MIN_ITEM_SUPPORT, min_session_length=MIN_SESSION_LENGTH ):
    data = load_data( path+file )
    
    data = filter_data( data, min_item_support, min_session_length )
   
    train, test = split_data_only( data, test_days )
...
def load_data( file ) :     
    #load csv
    data = pd.read_csv( file+'.dat', sep=',', header=None, usecols=[0,1,2,3], dtype={0:np.int32, 1:str, 2:np.int64})
    #specify header names
    data.columns = ['SessionId', 'TimeStr', 'ItemId', 'CatId']
...
def saveTrainData(data, path):
    fileName = "rec15_train_full"
    
    dataLen = int(len(data) / 64)
    ...
    rec15_train_full, rec15_test = split_data_only(data, 1)
    
    
    rec15_train_full.to_csv(path+"fulltrain/" + "rec15_train_full"+'.txt', sep='\t', index=False)
    rec15_test.to_csv(path+"fulltrain/" + "rec15_test"+'.txt', sep='\t', index=False)
...
def split_data_slice(data, output_file, slice_id, days_offset, days_train, days_test ) :
    ...
    # Start date___ it will be minimum date... 
    start = datetime.fromtimestamp( data.Time.min(), timezone.utc ) + timedelta( days_offset )
    # middle + start date become training data
    middle =  start + timedelta(days_train)
    # middle + one day training data becomes testing data
