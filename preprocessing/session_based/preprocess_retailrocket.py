...
DATA_PATH = r'./data/retailrocket/raw/'
DATA_PATH_PROCESSED = r'./data/retailrocket/prepare/'
DATA_FILE = 'events'
SESSION_LENGTH = 30 * 60 #30 minutes
ITEM_PROPERTIES1 = "item_properties_part1"
ITEM_PROPERTIES2 = "item_properties_part2"
IIEM_CATEGORIES = "retailrocket_category"
...
def preprocess_org_only( path=DATA_PATH, file=DATA_FILE, item_cate_dic = IIEM_CATEGORIES, path_proc=DATA_PATH_PROCESSED, min_item_support=MIN_ITEM_SUPPORT, min_session_length=MIN_SESSION_LENGTH, days_test=DAYS_TEST ):
    item_category_dict = load_data_make_dictionary1(path+item_cate_dic)
    data = load_data_only( path+file, item_category_dict)
    data = filter_data( data, min_item_support, min_session_length)
...
def load_data_make_dictionary():
    file1 = pd.read_csv(DATA_PATH+ITEM_PROPERTIES1+".csv")
    file2 = pd.read_csv(DATA_PATH+ITEM_PROPERTIES2+".csv")
    frame = [file1, file2]
...
def load_data( file ): 
    ...
    return data, cart;
def load_data_only( file , item_property_dic):
    #load csv
    data = pd.read_csv( file+'.csv', sep=',', header=0, usecols=[0,1,2,3], dtype={0:np.int64, 1:np.int32, 2:str, 3:np.int32})
    #specify header names
    data.columns = ['Time','UserId','Type','ItemId']
    ...
    for item in itemslist:
        if item in item_property_dic:
            catelist.append(item_property_dic[item])
        # put 0 if category of an item is not exist..... 
        else:
            count = count + 1
            catelist.append(0)     
    data["CatId"] = catelist
    ...
    data = data.loc[data["CatId"] > 0 ]
    return data

def load_data_make_dictionary1(file):
    dataset = pd.read_csv(file+".csv", sep =",")
    print("data shape:   ", dataset.shape)
    
    item_cate_dic = {}
    for i in range(len(dataset)):
        if dataset.iloc[i,0] not in item_cate_dic:
            item_cate_dic[dataset.iloc[i,0]] = dataset.iloc[i,1]
    print("Dictionary length:  ", len(item_cate_dic))
    return item_cate_dic
...
def split_data_only( data, days_test) :
...
def saveTrainData(data, path):
    filename = "retailrocket_train_full"
    train_tr = "retailrocket_train_tr"
    train_valid = "retailrocket_train_valid"
    
    tr, valid = split_data_only(data)
    tr.to_csv(path + train_tr+'.txt', sep='\t', index=False)
    valid.to_csv(path + train_valid+'.txt', sep='\t', index=False)
    data.to_csv(path+ + filename+'.txt', sep='\t', index=False)
    
    
        
def saveTestData(data, path):
    filename = "retailrocket_test"
    data.to_csv(path + filename+'.txt', sep='\t', index=False)
