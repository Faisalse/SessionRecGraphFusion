import importlib
# pathlib module is used handle the file path of different files. As we know
# different operating systems have different mechanism to handle the path of files.... 
from pathlib import Path
# sys package provides different functions to manipulate different parts of the python run time environment.....
import sys
import time
# os module provide functions to interact with operating systems....
import os
os.environ['KMP_DUPLICATE_LIB_OK']='True'
import glob
# traceback module is utilized to trace the execution of python file.... it tells where error is occured,,, so, we can easily find an error and fix
import traceback
# socket module is utilized to send information one node to another node,,,
import socket
import numpy as np
import pandas as pd
# this library is utilized to optimize the hyperparameters of ML algorithms.. 
...
def main(conf, out=None):
    ...
    # file contains config/in
    file = Path(conf)
    # print("File path  ",file)
    # is_file() is used to check file consists of file or something else. If it contains file then return true, otherwise, return false..
    if file.is_file():
        ...
        try:
            ...
            traceback.print_exc()
        exit()     
    # It starts runing from here. 
    # to check given path directory or something else. But it contains directory then return true otherwise it gives false answer.     
    if file.is_dir():
        if out is not None:
            ensure_dir(out + '/out.txt')
            send_message('tt waiting for configuration files in ' + conf)

            while True:
                print('waiting for configuration files in ', conf)
                # This function is utilized to match specific pattern.... and retrieve the files that matches specifc patterns..
                # this modules comes with python,,, so there is no need to install seperately. 
                
                # list = have name of file that is saved config/in folder... 
...
def run_single(conf, slice=None):
    ...
    # Fetch all algorithms from yaml file..data type: Dictionary
    algorithms = create_algorithms_dict(conf['algorithms'])
    # Fetch all metrics from yaml file: Dictionary
    metrics = create_metric_list(conf['metrics'])
    
    # Load evaluation which dictionary.... 
    ...
    if 'type' in conf['data']:
        if conf['data']['type'] == 'hdf':  # hdf5 file
            if 'opts' in conf['data']:
                ...
                train, test = dl.load_data_session_hdf(conf['data']['folder'], conf['data']['prefix'], slice_num=slice,                                                       **conf['data']['opts'])
            else:
    ...
    # ** TWO STARS IN defination is used to pass dictionary
        if 'opts' in conf['data']:
            print("This will be run if")
            # It returns training data with in dataframe format.... 
            train, test = dl.load_data_session(conf['data']['folder'], conf['data']['prefix'], slice_num=slice,
                                               **conf['data']['opts'])
            
        else:
            
            # Else part will not be run..... 
            train, test = dl.load_data_session(conf['data']['folder'], conf['data']['prefix'], slice_num=slice)
            # Not run in case of Yahoochoose.....
    ...
    for k, a in algorithms.items():
        # metrics is feed in form of list....
        print("---------")
    ...
    '''
     Perform an optmization for the algorithms
         ...
         conf: dict
             Configuration dictionary
     '''
    # Decide how many iterations, we should we perform
...
def eval_algorithm(train, test, key, algorithm, eval, metrics, results, conf, slice=None, iteration=None, out=True):    
    ...
    # hasattr is a built in function. This is a has-attribute function, If class has passed attribute, then it return true... Otherwise it returns false. 
    if hasattr(algorithm, 'init'):
        algorithm.init(train, test, slice=slice)
    
    for m in metrics:
        if hasattr(m, 'start'):
            # This starts function has nothing inside, just start time and end time... measure time... 
            m.start()
    
    start = timer()
    algorithm.fit(train, test)
    end = timer()
    print("Totla training time     ", end - start)
    ...
    if(key.split("-")[0] == "model3D"):
        results[key] = eval.evaluate_sessions_3model(algorithm, metrics, algorithm.test, train )
    

    else:
        if (key.split("-")[0] == "model3D_onlyInfo" ):
           results[key] = eval.evaluate_sessions(algorithm, metrics, algorithm.test, train)
           
        else:
            results[key] = eval.evaluate_sessions(algorithm, key.split("-")[0], metrics, test, train)
