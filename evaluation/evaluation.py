...
def evaluate_sessions(pr, key, metrics, test_data, train_data, items=None, cut_off=20, session_key='SessionId', item_key='ItemId', time_key='Time'): 
    ...
    for i in range(len(test_data)):
        ...
        if prev_sid != sid:
            ...
            preds = pr.predict_next(sid, prev_iid, items_to_predict, ts)
            for m in metrics:
...
def evaluate_sessions_CM_HGCN(pr, metrics, test_data, train_data, items=None, cut_off=20, session_key='SessionId', item_key='ItemId', time_key='Time'): 
    ...
    model, test_loader = pr.predict_next()
    
    
    
    for data in test_loader:
        pos = 0
        targets, scores = pr.forward(model, data)
        
        sub_scores_k100 = scores.topk(100)[1]
        sub_scores_k100 = trans_to_cpu(sub_scores_k100).detach().numpy()
        targets = targets.numpy()
        
        
        preds = pd.Series(data = list(sub_scores_k100[0]), index = list(sub_scores_k100[0]))
        ...
        for m in metrics:
            ...
            if hasattr(m, 'add'):
                m.add( preds, iid, for_item = 0, session = 0, position = pos)
...
def evaluate_sessions_SGINM(pr, metrics, test_data, train_data, items=None, cut_off=20, session_key='SessionId', item_key='ItemId', time_key='Time'): 
    ...
    model, test_data, slices = pr.predict_next()
    
    for i in range(len(slices)):
        te_inp_var, te_lengths, te_mask0, te_mask1, te_maskinf, te_out_var = test_data.batch2TrainData(slices[i])
        te_inp_var = te_inp_var
        te_lengths = te_lengths
        te_mask0 = te_mask0
        te_mask1 = te_mask1
        te_maskinf = te_maskinf
        test_score = model(te_inp_var, te_lengths, te_mask0, te_mask1, te_maskinf)
        
        sub_scores_k100 = test_score.topk(100)[1]
        sub_scores_k100 = sub_scores_k100.numpy()
        
        print("sub_scores_k100   ", type(sub_scores_k100))
    
    
        targets = te_out_var.numpy()
        preds = pd.Series(data = list(sub_scores_k100[0]), index = list(sub_scores_k100[0]))
        iid = targets[0]-1
        ...
        for m in metrics:
            if hasattr(m, 'add'):
                print("inside--------")
                m.add( preds, iid, for_item = 0, session = 0, position = 0)
...
def evaluate_sessions_3model(pr, metrics, train_data, test_data , items=None, cut_off=20, session_key='SessionId', item_key='ItemId', time_key='Time', CatId = "CatId", Brand	= "Brand"): 
    '''
    eval_algorithm
    Evaluates the baselines wrt. recommendation accuracy measured by recall@N and MRR@N. Has no batch evaluation capabilities. Breaks up ties.
    Parameters
    ...
    # Previous item id and session id....
    prev_iid, prev_cat, prev_brand,   prev_sid = -1, -1, -1, -1
    pos = 0
    ...
    for i in range(len(test_data)):
        ...
        iid = test_data[item_key].values[i]
        ts = test_data[time_key].values[i]
        brand = test_data[session_key].values[i]
        cat = test_data[item_key].values[i]
        ...
        if prev_sid != sid:
            ...
            # prediction starts from here.......
            preds = pr.predict_next(sid, prev_iid, prev_cat,  prev_brand)
...
def evaluate_sessions_side_sknn(pr, metrics, test_data, train_data, items=None, cut_off=20, session_key='SessionId', item_key='ItemId', time_key='Time', CatId = "CatId", Brand	= "Brand"): 
    '''
    eval_algorithm
    Evaluates the baselines wrt. recommendation accuracy measured by recall@N and MRR@N. Has no batch evaluation capabilities. Breaks up ties.
    Parameters
    ...
    for i in range(len(test_data)):
        ...
        iid = test_data[item_key].values[i]
        ts = test_data[time_key].values[i]
        brand = test_data[Brand].values[i]
        cat = test_data[CatId].values[i]
        ...
        if prev_sid != sid:
            ...
            preds = pr.predict_next(sid, prev_iid, prev_cat,  prev_brand, items_to_predict)
