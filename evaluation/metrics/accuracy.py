...
class CatD: 
    ...
    def init(self, train):
        ...
        '''
        Item_cat_dataframe = train[["ItemId", "CatId"]]
        self.item_cat_map = Item_cat_dataframe.groupby("ItemId")["CatId"].apply(list).to_dict()
        for key, value in self.item_cat_map.items():
            self.item_cat_map[key] = value[0]
        return 
    def reset(self):
        '''
    ...
    def add(self, result, next_item, for_item=0, session=0, pop_bin=None, position=None ):
        ...
        '''
        self.test += 1
        Unique_categoies = list()
        res = result[:self.length]
        for i in res.index:
            category = self.item_cat_map[i]
            if category not in Unique_categoies:
                Unique_categoies.append(category)
        self.uniCat+= len(Unique_categoies) 
        
    def result(self):
        '''
        Return a tuple of a description string and the current averaged value
        '''
        return ("CatD@" + str(self.length) + ": "), (self.uniCat/self.test)
