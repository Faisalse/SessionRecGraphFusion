...
def load_data(path, file, rows_train=None, rows_test=None, slice_num=None, density=1, train_eval=False):
    ...
    if (rows_test == None):
        ...
        test = pd.read_csv(path + file + test_appendix + split + '.txt' + density_appendix, sep='\t', nrows=rows_test)
...
def prepare_data_session(train, test, sessions_train=None, sessions_test=None, sess_type = None):
    ...
    if sess_type is None:
        ...
        if sess_type == "S" or sess_type == "s":
            test, session_length = short(test)
            print("Short sessions: ", session_length)
            
        if sess_type == "M" or sess_type == "m":
            test, session_length = medium(test)
            print("Medium sessions: ", session_length)
            
        if sess_type == "L" or sess_type == "l":
            test, session_length = long(test)
            print("long sessions: ", session_length)
...
def long(data):
    ...
    session_length = df1.groupby("SessionId").size()
    return df1, session_length
...
def load_data_session(path, file, sessions_train=None, sessions_test=None, sess_type = None, slice_num=None, train_eval=False):
    '''
    Loads a tuple of training and test set with the given parameters.
    ...
    # train and test dataframe and passsed into prepare_data_session..........
    train, test = prepare_data_session(train, test, sessions_train, sessions_test, sess_type)
...
def load_buys(path, file):
    ...
    # load csv
    buys = pd.read_csv(path + file + '.txt', sep='\t')
